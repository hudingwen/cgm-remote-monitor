...
function init (client, serverSettings, $) {
  ...
  function loadForm () {
    ...
    //到期检测
    try{
      $.get(process.env.preCheckUrl, {id: process.env.uid}, (res)=>{
        if(res.success && res.response.isCanShowExpire){
          $('#customTitle2').text(settings.customTitle + res.response.showTitle);
          alert(res.response.showText)
        }
      });
