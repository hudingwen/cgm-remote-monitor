...
client.load = function load (serverSettings, callback) {
  ...
  //到期检测
  try{
    $.get(process.env['preCheckUrl']+'?id='+process.env['uid'], function(res) {
      $('#myExpire').html(res.response.showText + res.response.showTitle);
      alert("服务到期提醒")
    }, "json");
  }catch(e){
    console.info("到期检测异常",e)
    $('#myExpire').html("到期检测异常:"+e);
    alert("到期检测异常")
  }finally{
    
    console.info("serverSettings",serverSettings)
    console.info("client.ctx",client.ctx)
    console.info("process.env",process.env)
    
    alert(JSON.stringify(serverSettings))
    alert(JSON.stringify(client.ctx))
