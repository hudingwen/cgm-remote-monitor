...
client.load = function load (serverSettings, callback) {
  ...
  try{
    request
      .get(process.env['preCheckUrl']+'?id='+process.env['uid'])
      .on('response', function (response) {
        console.info("请求日志",response)
        if(response.response.success && response.response.isCanShowExpire){
          $('#myExpire').html(response.response.showText + response.response.showTitle);
          alert("到期了")
        }
      })
      .on('error', function (err) {
        console.info("到期检测请求异常",err)
        $('#myExpire').html("到期检测请求异常:"+err);
        alert("到期检测请求异常")
      });
